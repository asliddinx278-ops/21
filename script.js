...
function addMessage(text, isUser = false) {
  ...
  div.className = isUser ? 'bubble-user rounded-lg rounded-tr-none px-3.5 py-2.5 max-w-[85%] text-sm ml-auto' : 'bubble-ai rounded-lg rounded-tl-none px-3.5 py-2.5 max-w-[85%] text-sm';
...
function showTyping() {
  ...
  div.className = 'bubble-ai rounded-lg rounded-tl-none px-3.5 py-2.5 max-w-[85%] text-sm flex gap-1 items-center';
...
const chatReplies = {
  ru: [
    "Понял. Уточните вес и размеры груза?",
    "Отлично. Какой срочности доставка вам нужна?",
    "Принято. Формирую RFQ и передаю менеджеру. Свяжемся в течение 2–4 часов.",
    "Спасибо за обращение! AI Cargo Assistant на связи 24/7."
  ],
  en: [
    "Got it. Could you clarify the weight and dimensions?",
    "Great. What urgency level do you need?",
    "Received. Building the RFQ and passing to a manager. We'll respond within 2–4 hours.",
    "Thank you for reaching out! AI Cargo Assistant is available 24/7."
  ],
  uz: [
    "Tushundim. Yuk og'irligi va o'lchamlarini ayting?",
    "Ajoyib. Qanday tezlikda yetkazib berish kerak?",
    "Qabul qilindi. RFQ tayyorlanmoqda. 2-4 soat ichida bog'lanamiz.",
    "Murojaatingiz uchun rahmat! AI Cargo Assistant 24/7 ishlaydi."
  ],
  zh: [
    "明白了。请确认货物的重量和尺寸？",
    "好的。您需要什么紧急程度？",
    "已收到。正在生成RFQ并转交给经理。我们将在2-4小时内回复。",
    "感谢您的联系！AI货运助手全天候在线。"
  ]
};
...
function handleChat() {
  ...
  setTimeout(() => {
    removeTyping();
    const replies = chatReplies[currentLang] || chatReplies.ru;
    addMessage(replies[chatStep % replies.length]);
    chatStep++;
  }, 1200);
}
...
document.getElementById('rfqForm')?.addEventListener('submit', function(e) {
  ...
  setTimeout(() => {
    ...
    // In production: send to API → PostgreSQL + Telegram + Email
  }, 1500);
});
